...
Standard tokens are generated by `npx shadcn init`. Dashboard extensions are injected from `scripts/templates/custom-tokens.css`.
...
fill="blue"
```
ALWAYS use `ChartTooltip` + `ChartTooltipContent` from `@/components/ui/chart`. Style CartesianGrid, XAxis, YAxis with token vars.
